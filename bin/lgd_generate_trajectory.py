...
def parse_command_line():
    ...
    parser.add_argument(
        "glowworm_id", help="glowworm to consider", type=int, metavar="glowworm_id"
    )
    parser.add_argument("steps", help="steps to consider", type=int, metavar="steps")
    parser.add_argument(
        "receptor_pdb",
        help="Receptor parsed PDB structure",
        type=valid_file,
        metavar="receptor_pdb",
    )
    parser.add_argument(
        "ligand_pdb",
        help="Ligand parsed PDB structure",
        type=valid_file,
        metavar="ligand_pdb",
    )
    parser.add_argument(
...
def parse_output_file(dock_output, glowworm_id, num_anm_rec, num_anm_lig):
    """Parses a simulation output file and returns only data for given glowworm_id"""
    with open(dock_output) as data_file:
    ...
    for step in range(0, args.steps + 1):
        try:
            ...
            if os.path.exists(file_name):
                translation, rotation, rec_extent, lig_extent = parse_output_file(
                    file_name, args.glowworm_id, num_anm_rec, num_anm_lig
                )
