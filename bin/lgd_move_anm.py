...
from Maindock.Param import DEFAULT_EXTENT_MU, DEFAULT_EXTENT_SIGMA
...
if __name__ == "__main__":
    ...
    dock_structure = Complex.from_structures(dock_structures)
    log.info("Structure parsed by Dock")
    ...
    num_atoms_dock = len(atoms)

    if num_atoms_prody != num_atoms_dock:
        log.error(f"Number of atoms is different between ProDy ({num_atoms_prody}) and Dock ({num_atoms_dock})")
        raise SystemExit
    ...
    coordinates = dock_structure.atom_coordinates[0].coordinates
    for i in range(args.n_confs):
        conf = (array * scale * randn[i]).sum(1).reshape((num_atoms_prody, 3))
        dock_structure.atom_coordinates[0].coordinates = coordinates + conf
        output_file = f"anm_{i+1}_{args.pdb_file}"
        write_pdb_to_file(dock_structure, output_file, dock_structure[0])
