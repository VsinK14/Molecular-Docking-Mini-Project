...
"""Generates the PDB structures given a  swarm results file"""
...
from Maindock.Param import (
    DEFAULT_NMODES_REC,
    DEFAULT_NMODES_LIG,
    DEFAULT_REC_NM_FILE,
    DEFAULT_LIG_NM_FILE,
)
from Maindock.pdbutil.PDBIO import parse_complex_from_file, write_pdb_to_file
from Maindock.structure.complex import Complex
from Maindock.Math.cython.quaternion import Quaternion
from Maindock.structure.nm import read_nmodes
from Maindock.prep.simulation import get_setup_from_file
from Maindock.util.parser import (
    valid_file,
...
def parse_output_file(dock_output, num_anm_rec, num_anm_lig):
    translations = []
    rotations = []
    receptor_ids = []
    ligand_ids = []
...
def parse_initial_file(dock_output, num_anm_rec, num_anm_lig):
    translations = []
    rotations = []
    receptor_ids = []
    ligand_ids = []
...
if __name__ == "__main__":
    ...
    # Receptor
    structures = []
    for structure in get_structures(args.receptor_structures):
    ...
    # Ligand
    structures = []
    for structure in get_structures(args.ligand_structures):
    ...
    # Output file
    output_file_path = Path(args.dock_output)
    if output_file_path.suffix == ".dat":
        (
            ...
            rec_extents,
            lig_extents,
        ) = parse_initial_file(args.dock_output, num_anm_rec, num_anm_lig)
    else:
        (
            translations,
            rotations,
            ...
            rec_extents,
            lig_extents,
        ) = parse_output_file(args.dock_output, num_anm_rec, num_anm_lig)
    ...
    for i in range(num_conformations):
        ...
        write_pdb_to_file(
            receptor,
            os.path.join(destination_path, "dock_%s.pdb" % i),
            receptor_pose,
        )
        write_pdb_to_file(
            ligand, os.path.join(destination_path, "dock_%s.pdb" % i), ligand_pose
        )
