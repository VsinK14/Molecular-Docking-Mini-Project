...
"""Before launching the simulation, a setup step is required.
...
import numpy as np
from Maindock.util.parser import SetupCommandLineParser
...
from Maindock.Math.ellipsoid import MinimumVolumeEllipsoid
...
if __name__ == "__main__":
    ...
    try:
        ...
        ellipsoid_data_file = "%s%s" % (
            DEFAULT_PREFIX % receptor.structure_file_names[0],
            DEFAULT_ELLIPSOID_DATA_EXTENSION,
        )
        ...
        ellipsoid_data_file = "%s%s" % (
            DEFAULT_PREFIX % ligand.structure_file_names[0],
            DEFAULT_ELLIPSOID_DATA_EXTENSION,
        )
        if not Path(ellipsoid_data_file).exists():
        ...
        save_structure(receptor)
        save_structure(ligand)
        ...
        log.info("Dock setup OK")

    except DockError as error:
        log.error("Dock setup failed. Please see:")
        log.error(error)
