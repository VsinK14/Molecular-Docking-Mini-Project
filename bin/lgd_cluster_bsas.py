...
"""Cluster final swarm results using BSAS algorithm"""
...
from Maindock.Param import CLUSTER_REPRESENTATIVES_FILE
...
def parse_command_line():
    ...
    parser.add_argument(
        "gso_output_file", help="Dock output file", metavar="gso_output_file"
    )
...
def get_backbone_atoms(ids_list, swarm_path):
    ...
    PDB files follow the format dock_ID.pdb where ID is in ids_list
    """
    ca_atoms = {}
    try:
        for struct_id in ids_list:
            pdb_file = swarm_path / f"dock_{struct_id}.pdb"
        ...
        log.error(
            "Did you generate the Dock structures corresponding to this output file?"
        )
        raise SystemExit()
    return ca_atoms
...
def clusterize(sorted_ids, swarm_path):
    ...
    for j in sorted_ids[1:]:
        log.info("Glowworm %d with pdb dock_%d.pdb" % (j, j))
        in_cluster = False
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Read output data
        gso_data = read_output(args.gso_output_file)
