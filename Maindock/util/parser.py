"""Module to parse the program command line options"""
    ...
    if input_file_extension == DEFAULT_LIST_EXTENSION:
        with open(input_file) as input_lines:
            for line in input_lines:
                ...
                dock_structure = os.path.join(
                    os.path.dirname(file_name),
                    DEFAULT_PREFIX % os.path.basename(file_name),
                )
                if os.path.exists(dock_structure):
                    file_names.append(dock_structure)
    else:
        ...
        if os.path.exists(dock_structure):
            ...
            raise DockError(f"Structure file {dock_structure} not found")
    return file_names
...
class SetupCommandLineParser(object):
    """Parses the command line of dock_setup"""

    def __init__(self, input_args=None):
        parser = argparse.ArgumentParser(prog="dock3_setup")
...
class CommandLineParser(object):
    ...
    def __init__(self, input_args=None):
        parser = argparse.ArgumentParser(prog="dock3")
