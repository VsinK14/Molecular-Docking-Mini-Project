"""Docking simulation using the multiprocessing library for parallelization"""
...
from Maindock.prep.simulation import (
    get_setup_from_file,
...
from Maindock.parallel.kraken import Kraken
from Maindock.parallel.util import GSOClusterTask
from Maindock.scoring.multiple import ScoringConfiguration
from Maindock.structure.nm import read_nmodes
from Maindock.error.app_error import NotSupportedInScoringError, SwarmNumError
...
def set_gso(
    ...
    """Creates a docking tool GSO simulation object"""
...
def set_scoring_function(parser, receptor, ligand):
    ...
    for scoring_function, weight in functions.items():
        ...
        scoring_function_module = "dock.scoring.%s.driver" % scoring_function
...
def run_simulation(parser):
    ...
    try:
        ...
        parsed_receptor = os.path.join(
            os.path.dirname(args.receptor_pdb),
            DEFAULT_PREFIX % os.path.basename(args.receptor_pdb),
        )
        ...
        parsed_ligand = os.path.join(
            os.path.dirname(args.ligand_pdb),
            DEFAULT_PREFIX % os.path.basename(args.ligand_pdb),
        )
        ligand = read_input_structure(
            parsed_ligand, args.noxt, args.noh, args.now, args.verbose_parser
        )
