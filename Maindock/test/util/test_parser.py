...
class TestParserUtils:
    ...
    def test_get_structures(self):
        ...
        structures = get_structures(list_of_pdbs)

        assert structures == [
            "dock_1czy_protein.pdb",
            "dock_1czy_peptide.pdb",
        ]

        # Test single structure
        single_pdb = self.golden_data_path / "1czy_protein.pdb"
        structures = get_structures(single_pdb)

        assert Path(structures[0]).name == "dock_1czy_protein.pdb"

    def test_valid_file_ok(self):
        os.chdir(self.golden_data_path)

        filename = valid_file("dock_1czy_protein.pdb")

        assert filename == "dock_1czy_protein.pdb"
