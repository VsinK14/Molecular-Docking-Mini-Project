...
class TestRegressionDNAShort(RegressionTest):
    ...
    def test_1diz_10_steps_20_glowworms(self):
        os.chdir(self.test_path)
        num_glowworms = 25
        steps = 10

        command = (
            f"dock3_setup.py 1DIZ_rec.pdb 1DIZ_lig.pdb -g {num_glowworms} -anm"
        )
        command += ">> test_dock.out"
        os.system(command)

        command = (
            f"dock3.py -c 1 -s dna setup.json {steps} -l 0 >> test_dock.out"
        )
        os.system(command)
...
class TestRegressionDNARestraints(RegressionTest):
    ...
    def test_1diz_30_steps_50_glowworms(self):
        os.chdir(self.test_path)
        num_glowworms = 50
        steps = 30

        command = f"dock3_setup.py 1DIZ_rec.pdb 1DIZ_lig.pdb -g {num_glowworms} -anm "
