...
class TestSetupWithMembrane(RegressionTest):
    ...
    def test_setup_with_membrane_automatic(self):
        os.chdir(self.test_path)

        command = "dock3_setup.py 3x29_receptor_membrane.pdb 3x29_ligand.pdb "
        command += "--noxt --noh -membrane -anm >> test_dock.out"
        os.system(command)
        ...
        assert filecmp.cmp(
            self.golden_data_path / "dock_3x29_receptor_membrane_mask.npy",
            self.test_path / "dock_3x29_receptor_membrane_mask.npy",
        )
        assert filecmp.cmp(
            self.golden_data_path / "dock_3x29_ligand_mask.npy",
            self.test_path / "dock_3x29_ligand_mask.npy",
        )
...
class TestSetupWithMembraneANM(RegressionTest):
    ...
    def test_setup_with_membrane_manual(self):
        os.chdir(self.test_path)

        num_swarms = 400
        num_glowworms = 50

        command = f"dock3_setup.py 3x29_receptor_membrane.pdb 3x29_ligand.pdb -g {num_glowworms} "
        command += f" -s {num_swarms} --noxt --noh -membrane >> test_dock.out"
        os.system(command)
        ...
        assert filecmp.cmp(
            self.golden_data_path / "dock_3x29_receptor_membrane.pdb",
            self.test_path / "dock_3x29_receptor_membrane.pdb",
        )
        assert filecmp.cmp(
            self.golden_data_path / "dock_3x29_ligand.pdb",
            self.test_path / "dock_3x29_ligand.pdb",
        )
...
class TestSetupWithMembraneAndRestraints(RegressionTest):
    ...
    def test_setup_with_membrane_automatic(self):
        ...
        command = f"dock3_setup.py 3x29_receptor_membrane.pdb 3x29_ligand.pdb -g {num_glowworms} "
        command += f" -s {num_swarms} --noxt --noh -membrane -rst restraints.list >> test_dock.out"
        os.system(command)
