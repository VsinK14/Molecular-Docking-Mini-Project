...
class TestRegressionPyDockRestraints(RegressionTest):
    ...
    def test_2uuy_20_steps_25_glowworms_rst(self):
        os.chdir(self.test_path)
        num_glowworms = 25
        steps = 20

        command = f"dock3_setup.py 1PPE_rec.pdb 1PPE_lig.pdb -g {num_glowworms} -anm "
        command += "-rst restraints.list >> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s cpydock setup.json {steps} -l 0 >> test_dock.out"
        os.system(command)
...
class TestRegressionCPyDockLong(RegressionTest):
    ...
    def test_dock_2uuy_40_steps_50_glowworms(self):
        os.chdir(self.test_path)
        num_glowworms = 50
        steps = 40

        command = f"dock3_setup.py 1PPE_rec.pdb 1PPE_lig.pdb -g {num_glowworms} >> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s cpydock setup.json {steps} -l 100 >> test_dock.out"
        os.system(command)
