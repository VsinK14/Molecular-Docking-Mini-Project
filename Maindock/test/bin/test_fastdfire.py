...
class TestRegressionFastDFIREShort(RegressionTest):
    ...
    def test_2uuy_5_steps_25_glowworms_100_swarms(self):
        os.chdir(self.test_path)
        num_swarms = 100
        num_glowworms = 25
        steps = 5

        command = f"dock3_setup.py 2UUY_rec.pdb 2UUY_lig.pdb -g {num_glowworms} -s {num_swarms} -anm"
        command += ">> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s fastdfire setup.json {steps} -l 10 >> test_dock.out"
        os.system(command)
...
class TestRegressionFastDFIRERestraints(RegressionTest):
    ...
    def test_2uuy_5_steps_25_glowworms_rst(self):
        os.chdir(self.test_path)
        num_glowworms = 25
        steps = 5

        command = (
            f"dock3_setup.py 2UUY_rec.pdb 2UUY_lig.pdb -g {num_glowworms} -anm "
        )
        command += "-rst restraints.list >> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s fastdfire setup.json {steps} -l 0 >> test_dock.out"
        os.system(command)
...
class TestRegressionFastDFIRELong(RegressionTest):
    ...
    def test_2uuy_40_steps_50_glowworms(self):
        os.chdir(self.test_path)
        num_glowworms = 50
        steps = 40

        command = f"dock3_setup.py 2UUY_rec.pdb 2UUY_lig.pdb -g {num_glowworms} >> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s fastdfire setup.json {steps} -l 100 >> test_dock.out"
        os.system(command)
...
class TestRegressionFastDFIRELocalMinimization(RegressionTest):
    ...
    def test_2uuy_2_steps_local_min(self):
        os.chdir(self.test_path)
        steps = 10
        swarm = 0

        command = "dock3_setup.py 2UUY_rec.pdb 2UUY_lig.pdb --noh --now --noxt -anm >> test_dock.out"
        os.system(command)

        command = f"dock3.py -c 1 -s fastdfire setup.json {steps} -l {swarm} -min >> test_dock.out"
        os.system(command)
