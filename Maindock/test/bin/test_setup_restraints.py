...
class TestSetupWithRestraints(RegressionTest):
    ...
    def test_setup_automatic(self):
        ...
        command = f"dock3_setup.py 2UUY_rec.pdb 2UUY_lig.pdb -g {num_glowworms} -anm --noxt --noh --now "
        command += "-rst restraints.list -sp >> test_dock.out"
        os.system(command)
...
class TestSetupWithRestraintsAndInsertCodes(RegressionTest):
    ...
    def test_setup_automatic(self):
        ...
        command = "dock3_setup.py receptor.pdb ligand.pdb --noh --noxt --now -anm "
        ...
        assert filecmp.cmp(
            self.golden_data_path / "dock_receptor.pdb",
            self.test_path / "dock_receptor.pdb",
        )
        assert filecmp.cmp(
            self.golden_data_path / "dock_ligand.pdb",
            self.test_path / "dock_ligand.pdb",
        )
...
class TestSetupWithRestraintsAndFlipMode(RegressionTest):
    ...
    def test_setup_automatic(self):
        ...
        command = "dock3_setup.py 3V6Z_A.pdb 3V6Z_B.pdb --noh --noxt -anm -flip "
        command += "-rst restraints.list >> test_dock.out"
        os.system(command)
