"""Regression tests for testing DFIRE scoring function"""
...
class TestRegressionDFIREShort(RegressionTest):
    ...
    def test_2uuy_10_steps_25_glowworms_100_swarms(self):
        ...
        command += f"-s dfire setup.json {steps} -l 0 >> test_dock.out"
        os.system(command)
...
class TestRegressionDFIRELong(RegressionTest):
    ...
    def test_dock_1ppe_50_steps_20_glowworms_100_swarms(self):
        os.chdir(self.test_path)
        num_swarms = 100
        num_glowworms = 20
        steps = 30
