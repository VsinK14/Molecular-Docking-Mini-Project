...
from Maindock.prep.poses import calculate_initial_poses
...
from Maindock.structure.nm import calculate_nmodes, write_nmodes
from Maindock.gso.Boundaries import Boundary, BoundingBox
from Maindock.error.app_error import DockError
from Maindock.version import CURRENT_VERSION


log = LoggingManager.get_logger("dock3_setup")
...
def save_structure(structure):
    """Saves the structure parsed by Docking tool"""
    ...
    for structure_index, file_name in enumerate(structure.structure_file_names):
        moved_file_name = Path(file_name).parent / Path(
            DEFAULT_PREFIX % Path(file_name).name
        )
...
def calculate_anm(structure, num_nmodes, rmsd, seed, file_name):
    ...
    # We have to use the parsed structure by Docking tool
    parsed_structure = Path(original_file_name).parent / Path(
        DEFAULT_PREFIX % Path(original_file_name).name
    )
    modes = calculate_nmodes(parsed_structure, num_nmodes, rmsd, seed, structure)
...
def prepare_results_environment(swarms=10):
    ...
    for id_swarm in range(swarms):
        ...
        if Path(saving_path).is_dir():
            raise DockError(f"Simulation folder {saving_path} already exists")
...
def create_simulation_info_file(args, path=".", file_name=DEFAULT_INFO):
...
def get_restraints(structure, restraints):
    ...
    for restraint_type in ["active", "passive", "blocked"]:
        for restraint in restraints[restraint_type]:
            ...
            if not residue:
                raise DockError(f"Restraint {restraint} not found in structure")
