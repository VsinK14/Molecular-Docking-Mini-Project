# A complex residue representation and operations

import numpy as np
from scipy.spatial import cKDTree
from error.app_error import (
    ResidueNonStandardError,
    SideChainError,
    BackboneError,
)
from Maindock.structure.atom import Atom
...
class Residue(object):
    #Representing a chemical residue in a complex
    ...
    def clone(self):
        #Creating a copy of the current residue
        return Residue(
            self.name,
            self.number,
            self.insertion,
    ...
    def is_standard(self):
        #Checking if residue is standard
        return self.name in list(Residue.STANDARD_TYPES.keys())

    def is_protein(self):
        #Checking if residue is protein
