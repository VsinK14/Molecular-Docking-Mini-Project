...
from Main.Math.cython.cutil import distance as cdistance
from error.app_error import AtomError


class Atom(object):
    #Represention of a chemical atom
    ...
    def __init__(
        ...
        """Creating a new atom. 
        Mass will be selected depending upon atom element. By default, creates a regular hydrogen atom. 
        Index can be used to quickly identify an atom or to use it as an index in an external data structure, i.e., a coordinates matrix."""
    ...
    def _assign_element(self):
        #Assigning an element to an atom depending on its name
    ...
    def is_hydrogen(self):
        #Checking if this atom is of hydrogen type
        return self.element == "H"

    def is_backbone(self):
        #Checking if this atom belongs to the residue backbone
        return self.name in Atom.BACKBONE_ATOMS

    def get_coordinates(self):
        #Getting the coordinates vector
        return [self.x, self.y, self.z]

    def clone(self):
        #Creating a copy of the current atom
        return Atom(
            self.number,
            self.name,
            self.alternative,
    ...
    def __eq__(self, other):
        """Comparing two atoms for equality. 
        Compare by number should be enough, but pdb files usually contain errors or numeration can be affected by chain id."""
        return (
    ...
    def __ne__(self, other):
        #Comparing two atoms for unequality"""
